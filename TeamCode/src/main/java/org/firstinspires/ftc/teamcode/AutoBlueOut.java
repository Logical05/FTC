...
@Config
@Autonomous(name="AutoBlueOut", group = "Out")
public class AutoBlueOut extends Robot {
    ...
    public void runOpMode() {
        ...
        if (opModeIsActive()) {
            ...
            Turn(90, 0.25);
            Move(0.8, 4.5, 0.23, 0, 1.21, 0.25, 0);
            if (signalPos == 'R') {
                Drop();
            }
            if (signalPos == 'L') {
                Move(0.7, 4.5, 0.23, -0.82, 1.2, 0.25, 0);
                Drop();
            }
            Move(0.9, 4.5, 0.23, -3.6, 1.2, 0.4, 0);
            Lift(800);
            if (signalPos == 'R') {
                Move(0.7, 4.5, 0.23, -3.6, 1.5, 0.25, 0);
            }
            if (signalPos == 'L') {
                Move(0.7, 4.5, 0.23, -3.6, 0.9, 0.25, 0);
            }
            Dropper();
            Lift(0);
            //            Move(0.7, 4.75, 0.23, 3.33, -0.25, 0.25, 0); // Corner
            Move(0.9, 4.75, 0.23, -3.65, 2.5, 0.25, 0);
        }
    }
}
